 ...
 * PIDOutput
 */
class PID1983Controller : public LiveWindowSendable, public Controller, public ITableListener
{
public:
	PID1983Controller(float p, float i, float d,
					PIDSource *source, PIDOutput *output,
					float period = 0.05);
	PID1983Controller(float p, float i, float d, float f,
					PIDSource *source, PIDOutput *output,
					float period = 0.05);
	virtual ~PID1983Controller();
	virtual float Get();
...
protected:
	...
	DISALLOW_COPY_AND_ASSIGN(PID1983Controller);
};
